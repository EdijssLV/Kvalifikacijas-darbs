    ...
    for url in shop_urls:
        ...
        while current_page:
            ...
            for product in products:
                ...
                if tilpums_raw:
                    tilpums_match = re.search(r"(\d+(?:\.\d*)?)l", tilpums_raw.string.replace(" ", ""), re.IGNORECASE)
                    tilpums = Decimal(tilpums_match.group(1).replace(",", ".")) if tilpums_match else None
                    tilpums = tilpums.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP) if tilpums else None
                else:
                    tilpums = None
                ...
                c.execute("""
                    INSERT INTO Kabinets (Name, Volume, Price, Store, Category, PricePerLiter)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (title, float(tilpums) if tilpums else None, float(cena) if cena else None, "AlkOutlet", None, cenaL))
    ...
    for url in shop_urls:
        ...
        while current_page:
            ...
            for product in products:
                ...
                price_div = soup.find("div", class_="price-tag card__price")
                try:
                ...
                c.execute("""
                    INSERT INTO Kabinets (Name, Volume, Price, Store, Category, PricePerLiter)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (title, float(tilpums) if tilpums else None, float(cena) if cena else None, "Rimi", None, float(cenaL) if cenaL else None))
            conn.commit()
        ...
        while current_page:
            ...
            for product in products:
                ...
                c.execute("""
                    INSERT INTO Kabinets (Name, Volume, Price, Store, Category, PricePerLiter)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (title, float(tilpums) if tilpums else None, float(cena) if cena else None, "Spirits & Wine", None, float(cenaL) if cenaL else None))
        ...
        while current_page:
            ...
            for product in products:
                ...
                c.execute("""
                   INSERT INTO Kabinets (Name, Volume, Price, Store, Category, PricePerLiter)
                   VALUES (?, ?, ?, ?, ?, ?)
                """, (title, float(tilpums) if tilpums else None, float(cena) if cena else None, "Latvijas Balzāms", None, float(cenaL) if cenaL else None))
